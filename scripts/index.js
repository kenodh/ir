...
function getObject(id) {
	...
	function scrollscroller(){ 
		if (iedom){ 
			if (parseInt(cross_scroller.style.top)>(actualheight*(-1)+8)) {
				...
				setScrollContent();
				setTimeout("startFadeEffect(102,133,204,'contentinfostrip','desktitle','deskphotos','deskvideos',titles[index],photos[index],videos[index])",100); //0,128,200
			}
		} 
		else if (document.layers){ 
			if (ns_scroller.top>(actualheight*(-1)+8)) {
				...
				scrollercontent = messages[index];
				setScrollContent();
				setTimeout("startFadeEffect(102,133,204,'contentinfostrip','desktitle','deskphotos','deskvideos',titles[index],photos[index],videos[index])",100);
			}
		}
	} 
	...
	function initializeHome()
	{
		...
		initilizeSlideShow();
		initializePrincipalsDesk();
		populate();
		if (titles.length > 0)
			setTimeout("startFadeEffect(102,133,204,'contentinfostrip','desktitle','deskphotos','deskvideos',titles[0],photos[0],videos[0])",100);
		else
			setTimeout("startFadeEffect(102,133,204,'contentinfostrip','desktitle','deskphotos','deskvideos','',0,0)",100);
		if (sitenews.length > 0) {
			setInterval("setNewsScrollerPos()",20);
			setInterval("updateNewsScroller()",3000);
		}
	}
	...
	function initializePrincipalsDesk() {
		if (iedom||document.layers){ 
			with (document){ 
				if (iedom){ 
					getObject('pricipalsdeskscroller').innerHTML = '<div style="position:relative;width:'+scrollerwidth+';height:'+scrollerheight+';overflow:hidden" onMouseover="copyspeed=pausespeed" onMouseout="copyspeed=scrollerspeed">' +
						'<div id="iescroller" style="position:absolute;left:0px;top:0px;width:100%;"></div></div>';
				} 
				else if (document.layers){ 
					getObject('pricipalsdeskscroller').innerHTML = '<ilayer width='+scrollerwidth+' height='+scrollerheight+' name="ns_scroller">' +
	...
	function updateNewsScroller() {
		if (sitenews.length > 0) {
			sitenewsindex = sitenewsindex + 1;
			if (sitenewsindex >= sitenews.length) sitenewsindex = 0;
			document.getElementById("newsscroleritems").innerHTML = sitenews[sitenewsindex];
		}
	}
	
	function setNewsScrollerPos() {
		var pos = 0;
		if (typeof(window.innerHeight) != 'undefined' && typeof(window.pageYOffset) != 'undefined') { 
			pos = parseInt(window.innerHeight) + parseInt(window.pageYOffset);
		}
		else if ((typeof(document.documentElement) != 'undefined') && (typeof(document.documentElement.scrollTop) != 'undefined')) {
			pos = parseInt(document.documentElement.clientHeight) + parseInt(document.documentElement.scrollTop);
		}
		else if (document.body) {
			pos = parseInt(document.body.clientHeight) + parseInt(document.body.scrollTop);
		}
		document.getElementById("newsscroler").style.top = (parseInt(pos) - document.getElementById("newsscroler").offsetHeight) + 'px';
	}
